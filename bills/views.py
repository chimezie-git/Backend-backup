...
class BuyAirtimeApI(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if result.is_success():
            saveTransaction(user, ref, f"{amount}", tType.airtime, provider, number, id=id)
...
class BuyDataApI(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if result.is_success():
            saveTransaction(user, ref, f"{amount}", tType.data, provider, int(plan_id), id=id)
            return Response(result.data, status=status.HTTP_200_OK)
        else:
            return Response(result.data, status=status.HTTP_400_BAD_REQUEST)
...
class PayElectricityApI(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if result.is_success():
            saveTransaction(user, ref, f"{amount}", tType.electricity, provider, number, id=id)
            return Response(result.data, status=status.HTTP_200_OK)
        else:
            return Response(result.data, status=status.HTTP_400_BAD_REQUEST)
...
class PayCableApI(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if result.is_success():
            saveTransaction(user, ref, f"{amount}", tType.cable, provider, number, id=id)
            return Response(result.data, status=status.HTTP_200_OK)
        else:
            return Response(result.data, status=status.HTTP_400_BAD_REQUEST)
...
class FundBettingApI(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if result.is_success():
            saveTransaction(user, ref, f"{amount}", f"{tType.betting.value}|{provider}", customer_id, id=id)
            return Response(result.data, status=status.HTTP_200_OK)
        else:
            return Response(result.data, status=status.HTTP_400_BAD_REQUEST)
...
class SendBulkSmsApI(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if result.is_success():
            ...
            saveTransaction(user, ref, f"{amount}", tType.bulk_sms, 'bulk_sms', '', id=id)
            return Response(result.data, status=status.HTTP_200_OK)
        else:
            return Response(result.data, status=status.HTTP_400_BAD_REQUEST)
